 ...
 * implementation of the edu.uw.cse.testbayes.Runner.tests331.RatPolyStack class.
 * 
...
public final class RatPolyStackTest {
  ...
  /** @return a new edu.uw.cse.testbayes.Runner.tests331.RatPolyStack instance */
  ...
  // edu.uw.cse.testbayes.Runner.tests331.RatPoly equality check
  // (getting around non-definition of edu.uw.cse.testbayes.Runner.tests331.RatPoly.equals)
  ...
  //
  // NOTE: THIS CAN FAIL WITH A WORKING STACK IF edu.uw.cse.testbayes.Runner.tests331.RatPoly.toString IS BROKEN!
  private void assertStackIs(RatPolyStack s, String desc) {
    Assert.assertEquals(desc.length(), s.size());
  ...
  public void testCtor() {
    ...
    Assert.assertEquals(0, stk1.size());
  }
  ...
  public void testDupWithTwoVal() {
    ...
    Assert.assertEquals(3, stk1.size());
  ...
  public void testDifferentiate() {
    ...
    stk1.differentiate();
    stk1.differentiate();
    Assert.assertEquals("Test if stack size changes", 3, stk1.size());
    ...
    stk1.differentiate();
    Assert.assertEquals("Test simple differentiate1", "4", stk1.pop().toString());
    stk1.differentiate();
    Assert.assertEquals("Test simple differentiate2", "0", stk1.pop().toString());
    stk1.differentiate();
    Assert.assertEquals("Test simple differentiate3", "15*x^4", stk1.pop().toString());
  }
  ...
  public void testIntegrate() {
    ...
    stk1.integrate();
    Assert.assertEquals("Test if stack size changes", 3, stk1.size());
    Assert.assertEquals("Test simple integrate1", "1/24*x^4", stk1.pop().toString());
    ...
    stk1.integrate();
    Assert.assertEquals("Test simple integrate1", "4*x", stk1.pop().toString());
    stk1.integrate();
    Assert.assertEquals("Test simple integrate2", "7*x", stk1.pop().toString());
    stk1.integrate();
    Assert.assertEquals("Test simple integrate3", "3*x^5", stk1.pop().toString());
  }
