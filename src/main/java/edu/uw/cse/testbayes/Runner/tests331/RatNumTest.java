 ...
 * implementation of the edu.uw.cse.testbayes.Runner.tests331.RatNum class.
 * <p>
 * edu.uw.cse.testbayes.Runner.tests331.RatNum is implemented for you, so it should already pass all the tests in
 * this suite. This test is provided to give you (1) examples of using the
 * edu.uw.cse.testbayes.Runner.tests331.RatNum class, albeit in the context of a test driver and (2) an example of a
 * test suite.
 * <p>
 */
...
public final class RatNumTest {
  ...
  /**
   * Asserts that edu.uw.cse.testbayes.Runner.tests331.RatNum.toString() is equal to rep. This method depends on the
   * implementation of edu.uw.cse.testbayes.Runner.tests331.RatNum's "toString" and "equals" methods. Therefore,
   * one should verify (test) those methods before using this method is in
   * tests.
   */
  private void eq(RatNum ratNum, String rep) {
    Assert.assertEquals(rep, ratNum.toString());
  }

  // The actual test cases are below.
  //
  // The order of the test cases is important for producing useful
  // output. If a test uses a method of edu.uw.cse.testbayes.Runner.tests331.RatNum, it should test that
  // method before hand. For example, suppose one of the test cases
  // for "negate" is:
  //
  // "(new edu.uw.cse.testbayes.Runner.tests331.RatNum(1)).negate().equals(new edu.uw.cse.testbayes.Runner.tests331.RatNum(-1))"
  //
  // In this case, the test case relies on edu.uw.cse.testbayes.Runner.tests331.RatNum's "equals" method
  ...
  // bug. (Furthermore, the test depends on edu.uw.cse.testbayes.Runner.tests331.RatNum's constructor,
  ...
  // it depends on are tested. For example, in edu.uw.cse.testbayes.Runner.tests331.RatNum, the "sub"
  ...
  // of edu.uw.cse.testbayes.Runner.tests331.RatTermTest and edu.uw.cse.testbayes.Runner.tests331.RatPolyTest, because you are proving the
  ...
  // depending on) all other edu.uw.cse.testbayes.Runner.tests331.RatNum methods.
  //
  // Unfortunately, without using any of edu.uw.cse.testbayes.Runner.tests331.RatNum's methods, all we
  ...
  // edu.uw.cse.testbayes.Runner.tests331.RatNum, r, where r.numer = 1 and r.denom = 1. Being a valid
  // edu.uw.cse.testbayes.Runner.tests331.RatNum, r would pass "checkRep" but would be the wrong edu.uw.cse.testbayes.Runner.tests331.RatNum
  // in most cases.
  //
  ...
  // testing.
  //
  // If edu.uw.cse.testbayes.Runner.tests331.RatNum had public fields, this problem would not exist,
  ...
  //
  // (2) Move the test suite code into edu.uw.cse.testbayes.Runner.tests331.RatNum and, thus, it would
  ...
  // fields of this class (edu.uw.cse.testbayes.Runner.tests331.RatNumTest); thus, one could argue that
  ...
  //NaN is Special instance and is non-intuitive, See edu.uw.cse.testbayes.Runner.tests331.RatNum Specification
  @Test
  ...
  // other edu.uw.cse.testbayes.Runner.tests331.RatNum methods (other than constructors).
  ...
  public void testIntValueUnsupported() {
    if (false) {
      // Note that these tests fail because our edu.uw.cse.testbayes.Runner.tests331.RatNum implementation
      ...
      Assert.assertEquals("(MIN_VALUE+1)/MIN_VALUE should round to 1",
          1, new RatNum(Integer.MIN_VALUE + 1, Integer.MIN_VALUE).intValue());
      Assert.assertEquals("1/MIN_VALUE should round to 0",
  ...
  public void testIntValueWithOutRound() {
    Assert.assertEquals("0 should round to 0", 0, zero.intValue());
    Assert.assertEquals("1 should round to 1", 1, one.intValue());
    Assert.assertEquals("-1 should round to -1", -1, negOne.intValue());
  }

  @Test
  public void testIntValueWithRoundtoPosOne() {
    Assert.assertEquals("1/2 should round to 1", 1, one_I_two.intValue());
    Assert.assertEquals("2/3 should round to 1", 1, two_I_three.intValue());
    Assert.assertEquals("3/4 should round to 1", 1, three_I_four.intValue());
  }

  @Test
  public void testIntValueWithRoundtoNegOne() {
    Assert.assertEquals("-1/2 should round to -1", -1, one_I_two.negate().intValue());
    Assert.assertEquals("-2/3 should round to -1", -1, two_I_three.negate().intValue());
    Assert.assertEquals("-3/4 should round to -1", -1, three_I_four.negate().intValue());
  }

  @Test
  public void testIntValueWithRoundMaxValue() {
    Assert.assertEquals("MAX_VALUE should round to MAX_VALUE",
        Integer.MAX_VALUE, new RatNum(Integer.MAX_VALUE).intValue());
    Assert.assertEquals("MAX_VALUE/2 should round to (MAX_VALUE/2)+1",
  ...
  public void testIntValueMinValueDivByMinValue() {
    Assert.assertEquals("MIN_VALUE/MIN_VALUE should round to 1",
  ...
  public void testEqualsReflexive() {
    for (int i = 0; i < ratNums.length; i++) {
      Assert.assertEquals(ratNums[i], ratNums[i]);
    }
  }
  ...
  public void testEqualsSimple() {
    ...
    Assert.assertEquals(one.add(one), two);
    // including negitives:
    Assert.assertEquals(negOne, negOne);
  }
  ...
  public void testEqualsSimpleWithDiffObjects() {
    ...
    // x.equals(y).
    Assert.assertEquals(new RatNum(1, 1), new RatNum(1, 1));
    Assert.assertEquals(new RatNum(1, 2), new RatNum(1, 2));
  }
  ...
  public void testEqualsNotReducedFormOne() {
    ...
    Assert.assertEquals(one, new RatNum(2, 2));
    Assert.assertEquals(new RatNum(2, 2), one);
  }

  @Test
  public void testEqualsNotReducedFormNegOne() {
    // including negitives:
    Assert.assertEquals(negOne, new RatNum(-9, 9));
    Assert.assertEquals(new RatNum(-9, 9), negOne);
  }

  @Test
  public void testEqualsNotReducedFormFraction() {
    // including double negitives:
    Assert.assertEquals(one_I_two, new RatNum(-13, -26));
    Assert.assertEquals(new RatNum(-13, -26), one_I_two);
  }

  @Test
  public void testEqualsNaN() {
    // Check that all NaN's are equals to one another.
    Assert.assertEquals(one_I_zero, one_I_zero);
    Assert.assertEquals(one_I_zero, negOne_I_zero);
    Assert.assertEquals(one_I_zero, hundred_I_zero);
  }
  ...
  // new edu.uw.cse.testbayes.Runner.tests331.RatNum(1, 2) as you will see below.)
  ...
  public void testCompareToNaNs() {
    for (int i = 0; i < ratNaNs.length; i++) {
      for (int j = 0; j < ratNaNs.length; j++) {
        Assert.assertEquals(ratNaNs[i], ratNaNs[j]);
      }
